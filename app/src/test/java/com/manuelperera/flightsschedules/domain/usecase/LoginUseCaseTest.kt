...
class LoginUseCaseTest {
    ...
    fun `invoke should return access token`() {
        ...
        whenever(loginRepository.login()).doReturn(getObEitherSuccess(accessToken))
        whenever(saveAccessTokenUseCase(any())).doReturn(getObEitherSuccess(accessToken))

        val testObserver = loginUseCase(None).test().await()
    ...
    fun `invoke should return failure`() {
        whenever(loginRepository.login()).doReturn(getObEitherError())
